    ...
    <div className="space-y-5">
      ...
      <Card title="You & your household" subtitle="Ages drive retirement, CPP and OAS timing.">
        <div className="space-y-4">
          {plan.people.map(p => {
            ...
            return (
              <div key={p.id} className={`rounded-container border p-4 ${on ? 'border-line' : 'border-dashed border-line-strong bg-surface-sunk/50'}`}>
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    <Users className="w-4 h-4 text-ink-muted" />
                    <span className="text-sm font-semibold text-ink">{PERSON_LABEL[p.id]}</span>
                  </div>
                  {p.id === 'partner' && (
        ...
        {plan.incomes.length === 0 ? (
          ...
          <div className="space-y-3">
            {plan.incomes.map(inc => (
              <div key={inc.id} className="border border-line rounded-container p-3">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    <Briefcase className="w-4 h-4 text-ink-muted" />
                    <span className="text-sm font-semibold text-ink">{inc.name || 'Income'}</span>
                  </div>
                  <button onClick={() => update({ incomes: plan.incomes.filter(i => i.id !== inc.id) })} className="p-1.5 text-ink-muted hover:text-negative hover:bg-negative-tint rounded-control"><Trash2 className="w-3.5 h-3.5" /></button>
                </div>
        ...
        <div className="grid lg:grid-cols-2 gap-5">
          <div className="space-y-3">
            <Field label="Everyday spending">
              <div className="flex gap-1 p-1 bg-surface-hover rounded-control text-caption font-medium mb-2">
                {[['history', 'From my history'], ['custom', 'Enter an amount']].map(([val, label]) => (
                  <button key={val} onClick={() => setLiving({ spendingMode: val })}
                    className={`flex-1 py-1.5 rounded-control transition-colors ${plan.living.spendingMode === val ? 'bg-surface  text-accent' : 'text-ink-muted hover:text-ink-secondary'}`}>
                    {label}
                  </button>
                ))}
              </div>
          ...
          <div className="grid grid-cols-2 gap-3 content-start">
            ...
            <div className="col-span-2 bg-surface-sunk rounded-container p-3 text-caption text-ink-secondary">
              <p className="flex items-center gap-1.5 font-medium text-ink-secondary mb-1"><Wallet className="w-3.5 h-3.5" />Starting money</p>
      ...
      <Card title="Account types" subtitle="Tax treatment for each account you track on the Investments page.">
        {state.investments.length === 0 ? (
          <p className="text-sm text-ink-muted text-center py-4">No investment accounts yet.</p>
        ) : (
              ...
              return (
                <div key={inv.id} className="flex flex-wrap items-center gap-3 border border-line rounded-container p-3">
                  <div className="flex-1 min-w-40">
                    ...
                    <p className="text-caption text-ink-muted">{formatCurrency(estimateAccountValue(inv).value)} estimated today</p>
                  </div>
      ...
      <Card title="Assumptions" subtitle="The projection is only as good as these. Conservative beats optimistic.">
        ...
        <p className="flex items-start gap-1.5 text-caption text-ink-muted mt-1">
