...
import type {
  AccountEntry, Debt, DerivedIncome, Income, Investment, IsoDate, Money,
} from '../types/domain';

/** Something that moved an account between its statement date and today. */
export interface AccountEvent {
  date: IsoDate;
  type: 'scheduled' | 'deposit' | 'withdrawal';
  amount: Money;
}

export interface AccountEstimate {
  /** False for an account with no statement date: its value is static. */
  tracked: boolean;
  value: Money;
  /** The statement balance everything is projected from. */
  anchorBalance: Money;
  anchorDate?: IsoDate;
  growth: Money;
  scheduledWithdrawn: Money;
  /** Withdrawals the user logged, as opposed to the standing schedule. */
  extraWithdrawn: Money;
  deposited: Money;
  events: AccountEvent[];
}
...
export interface AccountProjection {
  ...
  /** Month index when the balance hits zero, or null if it never does. */
  depletedMonth: number | null;
  depletionDate: IsoDate | null;
  /** The withdrawal growth alone could fund without shrinking the balance. */
  sustainableMonthly: Money;
  sustainable: boolean;
  /**
   * Balance at month `m`, clamped to the end of the timeline.
   *
   * A closure on an otherwise plain data object, which means this result is not
   * structured-cloneable — it can never be posted to a worker or sent over IPC.
   */
  balanceAt: (m: number) => Money;
}
...
const growthFactor = (annualPct: number | undefined, days: number): number =>
  Math.pow(1 + (Number(annualPct) || 0) / 100, days / 365);
const monthlyRate = (annualPct: number | undefined): number =>
  Math.pow(1 + (Number(annualPct) || 0) / 100, 1 / 12) - 1;
const daysBetween = (a: IsoDate, b: IsoDate): number =>
  Math.round((parseISO(b).getTime() - parseISO(a).getTime()) / DAY_MS);

export const isTrackedAccount = (inv: Investment | null | undefined): boolean => !!(inv && inv.asOfDate);
 ...
 */
export function entryCountsAfterStatement(inv: Investment, e: AccountEntry): boolean {
...
export function scheduledWithdrawalDates(inv: Investment, afterStr: IsoDate, untilStr: IsoDate): IsoDate[] {
 ...
 */
export function estimateAccountValue(inv: Investment, { today = new Date() }: { today?: Date } = {}): AccountEstimate {
  ...
  const events: AccountEvent[] = [
    ...scheduledWithdrawalDates(inv, anchor, todayStr).map((date): AccountEvent => ({ date, type: 'scheduled', amount: Number(inv.monthlyWithdrawal) })),
    // Entries on/before the statement date are already reflected in its balance.
    ...(inv.entries || []).filter(e => e.date <= todayStr && entryCountsAfterStatement(inv, e)).map((e): AccountEvent => ({ ...e, amount: Number(e.amount) || 0 })),
  ].sort((a, b) => a.date.localeCompare(b.date));

  let value = base;
  let cursor = anchor;
  let growth = 0, scheduledWithdrawn = 0, extraWithdrawn = 0, deposited = 0;
  const grow = (to: IsoDate): void => {
    const before = value;
...
export function getInvestmentsValue(investments: Investment[], { today = new Date() }: { today?: Date } = {}): Money {
...
export function projectAccount(
  inv: Investment,
  { today = new Date(), months = 360 }: { today?: Date; months?: number } = {},
): AccountProjection {
...
export function syncToStatement(
  inv: Investment,
  { balance, date, now = Date.now() }: { balance: Money; date: IsoDate; now?: number },
): Investment {
...
export function addAccountEntry(
  inv: Investment,
  { type, amount, date, note = '', now = Date.now() }: Omit<AccountEntry, 'id' | 'createdAt'> & { now?: number },
): Investment {
...
export function removeAccountEntry(inv: Investment, entryId: string): Investment {
 ...
 */
export function getIncomeSources(incomes: Income[], investments: Investment[]): (Income | DerivedIncome)[] {
  const fromAccounts = (investments || [])
    .filter(inv => Number(inv.monthlyWithdrawal) > 0 && inv.withdrawalCountsAsIncome !== false)
    .map((inv): DerivedIncome => ({
...
export function isInRepayment(debt: Debt, { today = new Date() }: { today?: Date } = {}): boolean {
...
export function requiredPayment(debt: Debt, opts?: { today?: Date }): Money {
...
export function monthsUntilRepayment(debt: Debt, { today = new Date() }: { today?: Date } = {}): number {
  if (isInRepayment(debt, { today }) || !debt.repaymentStart) return 0;
