    ...
    <div className="space-y-5">
              ...
              <div key={p.id} className={`rounded-xl border p-4 ${on ? 'border-gray-100' : 'border-dashed border-gray-200 bg-gray-50/50'}`}>
                ...
                {on && (
                  <div className="grid sm:grid-cols-3 lg:grid-cols-6 gap-3">
                    <NumberField label="Birth year" value={p.birthYear ?? ''} onChange={v => setPerson(p.id, { birthYear: v ? Number(v) : null })} placeholder="1999" />
                    <NumberField label="Retire at age" value={p.retireAge} onChange={v => setPerson(p.id, { retireAge: Number(v) })} />
                    <NumberField label="CPP at 65" value={p.cppAt65} onChange={v => setPerson(p.id, { cppAt65: Number(v) })} suffix="/yr" hint="Today's dollars. Max is about $17,200; average is closer to $9,000." />
                    <NumberField label="Start CPP at" value={p.cppStartAge} onChange={v => setPerson(p.id, { cppStartAge: Number(v) })} hint="60–70" />
                    <NumberField label="TFSA room today" value={p.tfsaRoom} onChange={v => setPerson(p.id, { tfsaRoom: Number(v) })} hint="Unused contribution room" />
                    <NumberField label="RRSP room today" value={p.rrspRoom} onChange={v => setPerson(p.id, { rrspRoom: Number(v) })} />
                    <NumberField label="FHSA per year" value={p.fhsaAnnual} onChange={v => setPerson(p.id, { fhsaAnnual: Number(v) })} suffix="/yr" hint="Up to $8,000/yr, $40,000 lifetime, for a first home" className="sm:col-span-2" />
                    <NumberField label="FHSA already contributed" value={p.fhsaContributedSoFar || 0} onChange={v => setPerson(p.id, { fhsaContributedSoFar: Number(v) })} />
                  </div>
                )}
              </div>
            );
            ...
            {plan.incomes.map(inc => (
              <div key={inc.id} className="border border-gray-100 rounded-xl p-3">
                ...
                <div className="grid sm:grid-cols-3 lg:grid-cols-7 gap-3">
                  ...
                  <NumberField label="Annual amount" value={inc.annual} onChange={v => setIncome(inc.id, { annual: Number(v) })} hint="Gross, before tax" />
                  <MonthField label="Starts" value={inc.start} onChange={v => setIncome(inc.id, { start: v })} />
                  <MonthField label="Ends (optional)" value={inc.end} onChange={v => setIncome(inc.id, { end: v })} hint="Blank = until retirement" />
                  <NumberField label="Raises" value={inc.growthPct} onChange={v => setIncome(inc.id, { growthPct: Number(v) })} suffix="%/yr" step="0.1" />
                  <NumberField label="RRSP contribution" value={inc.rrspPct} onChange={v => setIncome(inc.id, { rrspPct: Number(v) })} suffix="% of pay" step="0.5" />
                  <NumberField label="Employer match" value={inc.employerMatchPct} onChange={v => setIncome(inc.id, { employerMatchPct: Number(v) })} suffix="% of pay" step="0.5" />
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="space-y-3">
            <Field label="Everyday spending">
              ...
              {plan.living.spendingMode === 'history' ? (
                ...
                <NumberField label="Monthly spending" value={plan.living.spendingMonthly} onChange={v => setLiving({ spendingMonthly: Number(v) })} hint="Today's dollars, excluding rent and debt payments" />
              )}
            </Field>
            ...
            <div className="grid grid-cols-2 gap-3">
              <NumberField label="Rent" value={plan.living.rentMonthly} onChange={v => setLiving({ rentMonthly: Number(v) })} suffix="/mo" />
              <NumberField label="Rent increases" value={plan.living.rentGrowthPct} onChange={v => setLiving({ rentGrowthPct: Number(v) })} suffix="%/yr" step="0.1" />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3 content-start">
            <NumberField label="Spending in retirement" value={plan.living.retirementSpendingPct} onChange={v => setLiving({ retirementSpendingPct: Number(v) })} suffix="%" hint="Share of today's spending" />
            <NumberField label="Emergency buffer" value={plan.living.emergencyMonths} onChange={v => setLiving({ emergencyMonths: Number(v) })} suffix="months" hint="Kept in cash; the rest is invested" />
            <NumberField label="Cash on hand" value={plan.living.cashOnHand} onChange={v => setLiving({ cashOnHand: Number(v) })} hint="Chequing/savings not tracked as a goal or investment" className="col-span-2" />
      ...
      <Card title="Assumptions" subtitle="The projection is only as good as these. Conservative beats optimistic.">
        <div className="grid sm:grid-cols-3 lg:grid-cols-5 gap-3">
          <NumberField label="Inflation" value={plan.assumptions.inflationPct} onChange={v => setAssumptions({ inflationPct: Number(v) })} suffix="%/yr" step="0.1" />
          <NumberField label="Investment return" value={plan.assumptions.returnPct} onChange={v => setAssumptions({ returnPct: Number(v) })} suffix="%/yr" step="0.1" hint="Before inflation" />
          <NumberField label="Cash return" value={plan.assumptions.cashReturnPct} onChange={v => setAssumptions({ cashReturnPct: Number(v) })} suffix="%/yr" step="0.1" />
          <NumberField label="Home appreciation" value={plan.assumptions.homeAppreciationPct} onChange={v => setAssumptions({ homeAppreciationPct: Number(v) })} suffix="%/yr" step="0.1" />
          <NumberField label="Plan through age" value={plan.assumptions.endAge} onChange={v => setAssumptions({ endAge: Number(v) })} />
        </div>
        <p className="flex items-start gap-1.5 text-xs text-gray-400 mt-3">
